 ...
 * @LastEditors: Hanbing
 * @LastEditTime: 2024-10-12 17:12:01
 * @FilePath: /SAT/present/src/present128functions.cpp
 * @Description:
 *
 * Copyright (c) 2024 by Hanbing, All Rights Reserved.
 */
#ifndef PRESENT_MODE
#define PRESENT_MODE 128
#endif
...
void keySchedule(const bitset<KEY_SIZE> &master_key, int have_error, bitset<KEY_SIZE> ksr[ROUNDS], Util &myUtil) {
    ...
    for (int r = 0; r < ROUNDS - 1; r++) {
        ...
        for (int i = 0; i < KEY_SIZE; i++)
            k[i] = ksr[r][(i - 61 + KEY_SIZE) % KEY_SIZE];
        // k = (k << 61) | (k >> 19);
        bitset<4> s;
        ...
        for (int i = KEY_SIZE - 1; i >= KEY_SIZE - 4; i--)
            k[i] = s[i - (KEY_SIZE - 4)];
        index = (k[KEY_SIZE - 5] << 3) + (k[KEY_SIZE - 6] << 2) + (k[KEY_SIZE - 7] << 1) + k[KEY_SIZE - 8];
        s = myUtil.S(index, have_error);
        for (int i = KEY_SIZE - 5; i >= KEY_SIZE - 8; i--)
            k[i] = s[i - (KEY_SIZE - 8)];
        bitset<5> rc(r + 1);
        for (int i = KEY_SIZE / 2 + 2; i >= KEY_SIZE / 2 - 2; i--)
            k[i] = k[i] ^ rc[i - (KEY_SIZE / 2 - 2)];
        ksr[r + 1] = k;
    }
}
...
void genKsrConstraints(CMSat::SATSolver &solver, ClauseNode nodeKsr[][KEY_SIZE], Util &myUtil, string &msg) {
    ...
    for (int r = 0; r < ROUNDS - 1; r++) {
        ...
        // 2.2 循环左移61位之后的最高两个半字节127~124 123~120 进行字节代换
        ...
        for (int i = KEY_SIZE - (1 * ELEMENT_SIZE); i >= KEY_SIZE - (2 * ELEMENT_SIZE); i -= ELEMENT_SIZE) {
            // 2.2.1 CNF
        ...
        for (int i = 66; i >= 62; i--) {
            ...
            if (!rc[i - 62])
                continue;
            vector<unsigned> xor_vars;
            xor_vars.push_back(nodeKsr[r][(i - 61 + KEY_SIZE) % KEY_SIZE].newClauseNode(solver, NODE_CHECK_USED));
...
void genAKConstraints(CMSat::SATSolver &solver, int RC, int startRound, ClauseNode nodeKsr[][KEY_SIZE], ClauseNode nodeState[][3][BLOCK_SIZE], Util &myUtil, string &msg) {
    for (int i = BLOCK_SIZE - 1; i >= 0; i--) {
        ...
        xor_vars.push_back(nodeKsr[RC][i + KEY_SIZE - BLOCK_SIZE].newClauseNode(solver, NODE_CHECK_USED));
