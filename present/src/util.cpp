 ...
 * @LastEditors: Hanbing
 * @LastEditTime: 2024-09-29 17:12:38
 * @FilePath: /SAT/present/include/util.cpp
 * @Description:
 *
 ...
 * @return {*}
 */
Util::Util(int sbox_mode, string &msg) {
...
int Util::init_P(string &msg) {
    string funcName(__func__);
    msg = funcName;
    for (int i = 0; i < BLOCK_SIZE; i++) {
        P[i] = i * 16 % (BLOCK_SIZE - 1);
        if (i == (BLOCK_SIZE - 1))
            P[i] = i;
    }
    return 0;
...
int Util::getAnf(int index, vector<vector<int>> &anfSbox, int have_error, string &msg) {
    string funcName(__func__);
...
int Util::getCnf(vector<vector<pair<int, bool>>> &cnfSbox, int have_error, string &msg) {
    string funcName(__func__);
    switch (have_error) {
    case NO_ERROR:
...
void Util::getSboxInErrorIndex(bitset<4> &V) { V = this->sbox[sbox_error_index]; }
